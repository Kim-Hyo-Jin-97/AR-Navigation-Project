...
- 경기 인력개발원 유니티 부트캠프에서 약 2주간 진행했던 AR 네비게이션 제작 팀 프로젝트

# 담당 파트

- SI기업 '사람과 숲'과 협력하여 유니티를 이용해 인천시 재물포의 주요 장소(이하 POI)들을 소개하고 장소까지 가는 길을 알려주는 네비게이션 프로그램을 제작하였습니다.
- 총 4명이 함께 팀을 이루어 제작하였으며, 저는 길찾기 화면에 진입하면 원하는 지역의 지도와 사용자의 위치, POI의 데이터를 받아온 뒤 지도 화면을 불러오고 화면에 사용자와 POI의 위치에 맞게 마커를 띄워준 다음, POI의 마커를 띄우면 이와 관련된 정보를 보여주는 파트를 담당하였습니다.
...
- 3. POI의 위치에 맞게 지도상의 위치에 마커를 띄운다.
     2. 에서 사용한 API의 마커 기능을 활용할 계획입니다.
